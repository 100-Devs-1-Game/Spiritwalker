...
static var originMapData := MapData.new()
...
func load_or_download_tiles(_lat: float, _lon: float):
	...
	var actual_merc := Maths.mercatorProjection(_lat, _lon)
	var actual_uv := Maths.calculate_uv_from_merc(actual_merc)
	var our_tile_coords := Maths.calculate_tile_coordinate_from_uv(actual_uv)
	Maths.check_conversion(our_tile_coords)
...
func download_map_tilecoords(coords: Vector2i) -> void:
	...
	var uv := Maths.calculate_uv_from_tile_coordinate(coords)
	var merc := Maths.calculate_merc_from_uv(uv)
	var gps := Maths.inverseMercatorProjection(merc)
...
func downloadMap(_lat: float, _lon: float):
	...
	var actual_uv := Maths.calculate_uv_from_merc(actual_merc)
	var our_tile_coords := Maths.calculate_tile_coordinate_from_uv(actual_uv)
	var tile_bbox := Maths.calculate_tile_bounding_box_gps(our_tile_coords)
...
func parseAndReplaceMap(_file_path: String) -> MapData:
	...
	var player_current_tilecoords := Maths.calculate_tile_coordinate_from_uv(Maths.calculate_uv_from_merc(Maths.mercatorProjection(lat, lon)))
	...
	if not found_tile:
		...
		var offset := Maths.mercantorToGodotFromOrigin(mapData.boundaryData.center)
...
func playerBounds(x_merc: float, y_merc: float):
	var player_pos := Maths.mercantorToGodotFromOrigin(Vector2(x_merc, y_merc))
