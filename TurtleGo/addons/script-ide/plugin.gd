...
## Editor setting for disabling the whole plugin
const ENABLED_SETTING: StringName = SCRIPT_IDE + &"enabled"
#endregion
...
func _enter_tree() -> void:
	...
	if is_enabled == null:
		editor_settings.set_setting(ENABLED_SETTING, false)
	if not is_enabled:
		return
...
func sync_settings():
	...
	for setting: String in changed_settings:
		...
		if setting == OUTLINE_POSITION_RIGHT:
			...
			var new_script_tabs_visible: bool = get_setting(
				SCRIPT_TABS_VISIBLE, is_script_tabs_visible
			)
