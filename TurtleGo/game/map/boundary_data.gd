...
# README
# - this was one of the first thing I added and it could do with a little cleanup
# - it basically tries to store all the size data for a tile
...
func _init(
	...
	if minimum != Vector2.ZERO && maximum != Vector2.ZERO && center != Vector2.ZERO:
		valid = true
	...
	var coords_new := Maths.calculate_tile_coordinate_from_uv(
		Maths.calculate_uv_from_tile_coordinate(tile_coordinate)
	)
